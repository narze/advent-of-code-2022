...
def solve(lines):
    ...
    mod = functools.reduce(lambda a,b: a * b, [x["exp"] for x in entries])

    for nth in range(10000):
        for i, entry in enumerate(entries):
            ...
            for old in entry["items"]:
                ...
                if new_level % entry["exp"] == 0:
                    entries[entry["true"]]["items"].append(new_level % mod)
                else:
                    entries[entry["false"]]["items"].append(new_level % mod)

                entries[i]["inspects"] += 1
                entries[i]["items"] = []
        print(nth, [len(x["items"]) for x in entries])
